...
describe("form array " + start_async_validation_1.startAsyncValidationReducer.name, function () {
    ...
    it('should mark state as having validation pending if control child is marked as having validation pending', function () {
        var name = 'required';
        var resultState = start_async_validation_1.startAsyncValidationReducer(test_util_1.INITIAL_STATE, new actions_1.StartAsyncValidationAction(test_util_1.FORM_CONTROL_0_ID, name));
    ...
    it('should mark state as having validation pending if group child is marked as having validation pending', function () {
        var name = 'required';
        var resultState = start_async_validation_1.startAsyncValidationReducer(test_util_1.INITIAL_STATE_NESTED_ARRAY, new actions_1.StartAsyncValidationAction(test_util_1.FORM_CONTROL_0_ID, name));
        expect(resultState.isValidationPending).toEqual(true);
    });
    it('should mark state as having validation pending if array child is marked as having validation pending', function () {
        var name = 'required';
        var resultState = start_async_validation_1.startAsyncValidationReducer(test_util_1.INITIAL_STATE_NESTED_GROUP, new actions_1.StartAsyncValidationAction(test_util_1.FORM_CONTROL_0_ID, name));
        expect(resultState.isValidationPending).toEqual(true);
    });
});
