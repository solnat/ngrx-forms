...
describe("form array " + mark_as_pristine_1.markAsPristineReducer.name, function () {
    ...
    it('should mark state as pristine if all children are pristine when control child is updated', function () {
        var state = state_1.cast(test_util_1.setPropertiesRecursively(test_util_1.INITIAL_STATE, [['isDirty', true], ['isPristine', false]], test_util_1.FORM_CONTROL_0_ID));
        var resultState = mark_as_pristine_1.markAsPristineReducer(state, new actions_1.MarkAsPristineAction(test_util_1.FORM_CONTROL_1_ID));
        expect(resultState.isDirty).toEqual(false);
        expect(resultState.isPristine).toEqual(true);
    });
    it('should not mark state as pristine if not all children are pristine when control child is updated', function () {
        var state = state_1.cast(test_util_1.setPropertiesRecursively(test_util_1.INITIAL_STATE, [['isDirty', true], ['isPristine', false]]));
        var resultState = mark_as_pristine_1.markAsPristineReducer(state, new actions_1.MarkAsPristineAction(test_util_1.FORM_CONTROL_0_ID));
    ...
    it('should mark state as pristine if all children are pristine when group child is updated', function () {
        var state = state_1.cast(test_util_1.setPropertiesRecursively(test_util_1.INITIAL_STATE_NESTED_GROUP, [['isDirty', true], ['isPristine', false]], test_util_1.FORM_CONTROL_0_ID));
        var resultState = mark_as_pristine_1.markAsPristineReducer(state, new actions_1.MarkAsPristineAction(test_util_1.FORM_CONTROL_1_ID));
        expect(resultState.isDirty).toEqual(false);
        expect(resultState.isPristine).toEqual(true);
    });
    it('should mark state as pristine if all children are pristine when array child is updated', function () {
        var state = state_1.cast(test_util_1.setPropertiesRecursively(test_util_1.INITIAL_STATE_NESTED_ARRAY, [['isDirty', true], ['isPristine', false]], test_util_1.FORM_CONTROL_0_ID));
        var resultState = mark_as_pristine_1.markAsPristineReducer(state, new actions_1.MarkAsPristineAction(test_util_1.FORM_CONTROL_1_ID));
        expect(resultState.isDirty).toEqual(false);
        expect(resultState.isPristine).toEqual(true);
    });
